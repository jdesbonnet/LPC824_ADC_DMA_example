...
*/
	...
	for (i =0; i < 100000000; i++) {
		...
		for (j = 0; j < 23; j++) {
			...
			//Chip_SPI_SendMidFrame(LPC_SPI0, TxBuf[j]);
		}
		...
		/*
		Chip_SPI_SendFirstFrame(LPC_SPI0, 0x55, 8);
		Chip_SPI_SendMidFrame(LPC_SPI0,0xAA);
		Chip_SPI_SendLastFrame_RxIgnore(LPC_SPI0, 0x55,8);
		*/
		//LPC_SPI0->TXDAT = 0x55;
		...
		/*
		while (! Chip_SPI_GetStatus(LPC_SPI0) & SPI_STAT_TXRDY) {};
		Chip_SPI_WriteTXData(LPC_SPI0,0x55);
		Chip_SPI_ReadRXData(LPC_SPI0);
		*/
	}
