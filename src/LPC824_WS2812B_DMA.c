...
int main(void) {
	...
	//dmaDescB.dest = DMA_ADDR(&adc_buffer[DMA_BUFFER_SIZE*3 - 1]) ;
	...
	// Lets try with bit banging first
	const int N = 4;
	const int T0 = 1;
	const int T1 = 2;
	...
	while (1) {
		...
		for (j = 0; j < 16; j++) {
			...
			for (i = 0; i < 24; i++) {
				Chip_GPIO_SetPinState(LPC_GPIO_PORT, 0, PIN_DEBUG, 1);
				if (color & 1) {
					__NOP();
					__NOP();
					__NOP();
					...
					__NOP();
					__NOP();
					Chip_GPIO_SetPinState(LPC_GPIO_PORT, 0, PIN_DEBUG, 0);
				} else {
					__NOP();
					__NOP();
					Chip_GPIO_SetPinState(LPC_GPIO_PORT, 0, PIN_DEBUG, 0);
